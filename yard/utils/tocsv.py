...
def dataframe_from_db(code, column, start_date, end_date):
    ...
    con = mdb.connect(host=db_info['host'],
                      ...
                      db=db_info['database'])
    df = _dataframe_from_db(con, code, column, start_date, end_date)
    return df


def dataframes_from_db(codes, column, start_date, end_date):
    db_info = settings.PERIODIC_QUOTE_DATABASE
    con = mdb.connect(host=db_info['host'],
                      port=db_info['port'],
                      user=db_info['user'],
...
def _dataframe_from_db(con, code, column, start_date, end_date):
